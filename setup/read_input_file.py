...
from utilities.utils import stratifiedgroupsplit_train_val, stratifiedgroupsplit, stratified_class_count, groupby_view_train, \
    stratifiedgroupsplit, groupby_view_test, stratifiedgroupsplit_train_test
...
def input_file_creation(config_params):
    ...
    if config_params['learningtype'] == 'SIL':
        if config_params['datasplit'] == 'officialtestset':
            ...
            if config_params['usevalidation']:
                df_train, df_val, df_test = stratifiedgroupsplit(df_modality1, config_params['randseeddata'], patient_col)
            else:
                df_train, df_test = stratifiedgroupsplit_train_test(df_modality1, config_params['randseeddata'], patient_col)
            ...
            if config_params['usevalidation']:
                df_train1, df_val1, df_test1 = stratifiedgroupsplit(df_modality2, config_params['randseeddata'], patient_col)
            else:
                df_train1, df_test1 = stratifiedgroupsplit_train_test(df_modality2, config_params['randseeddata'], patient_col)
    ...
    if config_params['viewsinclusion'] == 'all' and config_params['learningtype'] == 'MIL':
        #group by view
        view_group_indices, view_group_names_train = groupby_view_train(df_train)
        print(view_group_names_train)
        
        df_val, view_group_names_val = groupby_view_test(df_val)
        print(view_group_names_val)
        
        df_test, view_group_names_test = groupby_view_test(df_test)
