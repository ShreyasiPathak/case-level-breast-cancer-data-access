...
from utilities.data_loaders_utils import collect_images, collect_cases
from utilities.gmic_utils import get_crop_mask

from train_eval.test import load_model_for_testing

from analysis.mask_roi_match import match_to_mask_images_cbis, match_to_mask_images_vindr, match_to_mask_images_zgt, roi_diagnosis_images_vindr
from analysis.imagelabel_attwt_match import extract_img_attn_wts
...
def seg_evaluation(config_params, input_img_case, img_path_list, patch_locations_case, patch_img_case, patch_attentions_case, img_attention_case, views_names, exam_name, df_score, seg_eval_metric, pred_label, true_label, roi_diagnosis_conf_mat_all):    
    ...
    if not not_allow:
        ...
        for view_id, view_name in enumerate(views_names):
            ...
            if config_params['dataset'] == 'cbis-ddsm':
                iou_view_each_roi, _, iou_highestattnwt_each_roi, fig, ax = match_to_mask_images_cbis(config_params, input_img, image_name, patch_attentions, patch_locations, seg_eval_metric, view_id, views_names, fig, ax)
            elif config_params['dataset'] == 'vindr':
                ...
                iou_view_each_roi, _, iou_highestattnwt_each_roi, fig, ax, roi_diagnosis_conf_mat_percase = roi_diagnosis_images_vindr(config_params, input_img, image_name, patch_attentions, patch_locations, seg_eval_metric, view_id, views_names, fig, ax, pred_label, true_label, roi_diagnosis_conf_mat_percase)
            elif config_params['dataset'] == 'zgt':
                iou_view_each_roi, _, iou_highestattnwt_each_roi, fig, ax = match_to_mask_images_zgt(config_params, input_img, image_name, patch_attentions, patch_locations, seg_eval_metric, view_id, views_names, fig, ax, img_attention)
        ...
        for test_idx, test_batch, test_labels, views_names in dataloader_test:
            ...
            if config_params['femodel'] == 'gmic_resnet18':
                ...
                if config_params['learningtype'] == 'SIL':
                    loaded_image = collect_images(config_params, df_test.loc[test_idx.item()])
                    ...
                    if config_params['dataset'] == 'cbis-ddsm':
                        ...
                        exam_name = df_test.loc[test_idx.item(), 'ShortPath'].split('/')[-1]
                    loaded_image, _, _, img_path_list = collect_cases(config_params, df_test.loc[test_idx.item()])
            ...
            if config_params['femodel'] == 'gmic_resnet18':
                if config_params['learningtype'] == 'SIL':
                    ...
                    if config_params['dataset'] == 'cbis-ddsm':
                        ...
                        exam_name = df_test.loc[test_idx.item(), 'StudyInstanceUID']
                    loaded_image, _, _, _ = collect_cases(config_params, df_test.loc[test_idx.item()])
                    _, _, output_batch_fusion, saliency_map, patch_locations, patch_imgs, patch_attns, img_attns, _, _ = model(test_batch, views_names, eval_mode)
                    patch_locations = patch_locations.cpu()
                    patch_imgs = patch_imgs.cpu()
