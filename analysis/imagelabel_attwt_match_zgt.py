...
from train_eval.evaluation import write_results_xlsx_confmat, write_results_xlsx, aggregate_performance_metrics
...
def model_output(config_params, model, dataloader_test, df_test, path_to_results):
    ...
    if config_params['learningtype'] == 'MIL':
        if config_params['milpooling'] == 'ismean' or config_params['milpooling'] == 'isatt' or config_params['milpooling'] == 'isgatt':
            ...
            write_results_xlsx_confmat(config_params, conf_mat_insprob, path_to_results, 'imglabel_confmat')
            ...
            per_model_metric = aggregate_performance_metrics(config_params, y_true_all, y_pred_insprob_all, None)
            print(per_model_metric)
            write_results_xlsx(per_model_metric, path_to_results, 'imglabel_confmat')
            ...
            write_results_xlsx_confmat(config_params, conf_mat_attwt, path_to_results, 'imglabel_confmat')
            wb = op.load_workbook(path_to_results)
            sheet = wb['imglabel_confmat']
            sheet.append(['PrecisionBin','PrecisionMicro','PrecisionMacro','RecallBin','RecallMicro','RecallMacro','F1Bin','F1Micro','F1macro','F1wtmacro','Acc','Cohens Kappa','AUC'])
            wb.save(path_to_results)
            per_model_metric = aggregate_performance_metrics(config_params, y_true_all, y_pred_attwt_all, None)
            print(per_model_metric)
            write_results_xlsx(per_model_metric, path_to_results, 'imglabel_confmat')
            ...
            write_results_xlsx_confmat(config_params, conf_mat_insimp, path_to_results, 'imglabel_confmat')
            wb = op.load_workbook(path_to_results)
            sheet = wb['imglabel_confmat']
            sheet.append(['PrecisionBin','PrecisionMicro','PrecisionMacro','RecallBin','RecallMicro','RecallMacro','F1Bin','F1Micro','F1macro','F1wtmacro','Acc','Cohens Kappa','AUC'])
            wb.save(path_to_results)
            per_model_metric = aggregate_performance_metrics(config_params, y_true_all, y_pred_insimp_all, None)
            print(per_model_metric)
            write_results_xlsx(per_model_metric, path_to_results, 'imglabel_confmat')
            ...
            print(conf_mat)
            write_results_xlsx_confmat(config_params, conf_mat, path_to_results, 'imglabel_confmat')
            print(count)
            wb = op.load_workbook(path_to_results)
            sheet = wb['imglabel_confmat']
            sheet.append(['PrecisionBin','PrecisionMicro','PrecisionMacro','RecallBin','RecallMicro','RecallMacro','F1Bin','F1Micro','F1macro','F1wtmacro','Acc','Cohens Kappa','AUC'])
            wb.save(path_to_results)
            per_model_metric = aggregate_performance_metrics(config_params, y_true_all, y_pred_all, None)
            print(per_model_metric)
            write_results_xlsx(per_model_metric, path_to_results, 'imglabel_confmat')
    elif config_params['learningtype'] == 'SIL':
        y_true_all = np.array(y_true_all)
