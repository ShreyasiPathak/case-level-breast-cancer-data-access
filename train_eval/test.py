...
from train_eval.loss_function import loss_fn_bce, loss_fn_crossentropy, loss_fn_gmic, loss_fn_gmic_initialize
from train_eval.evaluation import write_results_xlsx, write_results_xlsx_confmat, conf_mat_create, calc_viewwise_metric, results_abnormality, calc_viewwise_metric_newplot, \
    aggregate_performance_metrics, classspecific_performance_metrics, write_results_viewwise, results_breastdensity, results_birads, case_label_from_SIL
...
def test(config_params, model, dataloader_test, batches_test, df_test, path_to_results_xlsx, sheetname, epoch):
    ...
    if config_params['femodel'] == 'gmic_resnet18':
        bcelogitloss, bceloss = loss_fn_gmic_initialize(config_params, None, test_bool=True)
    else:
        if config_params['activation'] == 'softmax':
            lossfn1 = loss_fn_crossentropy(config_params, None, test_bool=True)
        elif config_params['activation'] == 'sigmoid':
            lossfn1 = loss_fn_bce(config_params, None, test_bool=True)
    ...
    with torch.no_grad():
        for test_idx, test_batch, test_labels, views_names in dataloader_test:
            ...
            if config_params['femodel'] == 'gmic_resnet18':
                ...
                loss1 = loss_fn_gmic(config_params, bcelogitloss, bceloss, output_batch_local, output_batch_global, output_batch_fusion, saliency_map, test_labels, None, output_patch_test, test_bool=True).item()
            ...
            correct, total_images, conf_mat_test, conf_mat_batch = conf_mat_create(test_pred, test_labels, correct, total_images, conf_mat_test, config_params['classes'])
            ...
            if config_params['viewsinclusion'] == 'all' and config_params['learningtype'] == 'MIL' and (config_params['dataset'] == 'zgt' or config_params['dataset'] == 'cbis-ddsm'):
                count_dic_viewwise = calc_viewwise_metric_newplot(views_names, views_standard, count_dic_viewwise, test_labels, test_pred, output_test)
    ...
    per_model_metrics = aggregate_performance_metrics(config_params, test_labels_all.cpu().numpy(),test_pred_all.cpu().numpy(), output_all_ten.cpu().numpy())
    ...
    if sheetname == 'hyperparam_results':
        ...
        write_results_xlsx(hyperparam_details, config_params['path_to_hyperparam_search'], 'hyperparam_results')
    else:
        ...
        classspecific_performance_metrics(config_params, test_labels_all.cpu().numpy(),test_pred_all.cpu().numpy(), output_all_ten.cpu().numpy(), path_to_results_xlsx, 'test_results')
    
        if config_params['viewsinclusion'] == 'all' and config_params['learningtype'] == 'MIL' and (config_params['dataset'] == 'zgt' or config_params['dataset'] == 'cbis-ddsm'):
            write_results_viewwise(config_params, path_to_results_xlsx, 'metrics_view_wise', count_dic_viewwise)
        
        if eval_subgroup:
            results_breastdensity(config_params, df_test, test_labels_all.cpu().numpy(), test_pred_all.cpu().numpy(), output_all_ten.cpu().numpy(), path_to_results_xlsx)
            results_birads(config_params, df_test, test_labels_all.cpu().numpy(), test_pred_all.cpu().numpy(), output_all_ten.cpu().numpy(), path_to_results_xlsx)
        ...
        if config_params['learningtype'] == 'SIL':
            case_label_from_SIL(config_params, df_test, test_labels_all.cpu().numpy(), test_pred_all.cpu().numpy(), path_to_results_xlsx)
...
def run_test(config_params, model, path_to_model, dataloader_test, batches_test, df_test, path_to_results_xlsx, sheetname, epoch):
    ...
    '''
    To log model through mlflow, uncomment the following line. But for this to work, mlflow server should allow logging of artifacts.
    '''
